  ...
  try {
    if (!process.env.MONGO_URI) {
      return NextResponse.json(
        { message: "Konfiguracija baze podataka nije ispravna." },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      { message: "Greška pri povezivanju sa bazom podataka." },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      message: "Podaci su uspešno kreirani.",
      id: result.insertedId,
    });
  } catch (error) {
    return NextResponse.json(
      { message: "Greška pri kreiranju podataka." },
      { status: 500 },
    );
  }
}
